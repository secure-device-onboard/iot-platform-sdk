...
public class Keys {
  ...
  /**
   * Utility method to return the size (in bytes) of the given key.
   *
   * @param key {@link Key}
   * @return size of the key
   */
  public static int sizeInBytes(Key key) {
   ...
   */
  public static PublicKey toPublicKey(KeySpec keySpec) throws NoSuchAlgorithmException {
   ...
   */
  public static KeyType toType(PublicKey key) {
  ...
  private static Set<ASN1ObjectIdentifier>
      findObjectIdentifiers(final ASN1Encodable asn1Encodable) {
  ...
  private static boolean areEqualByValue(final ECParameterSpec left, final ECParameterSpec right) {
    ...
    return !(null == left || null == right) && left.getCurve().equals(right.getCurve())
