 ...
 */
public class EpidLib implements AutoCloseable {
   ...
   */
  public EpidLib() throws IOException {}
  ...
  public byte[] getEpidInfo11_eB(byte[] groupId)
    ...
    Epid11eB eb = new Epid11eB(groupId);
    return eb.getArray();
  }
  ...
  public byte[] getEpidInfo20_eB(byte[] groupId)
    ...
    if (groupId.length != EpidConstants.EPID2X_GID_SIZE) {
      mlog.error("invalid groupId length: " + groupId.length + ", expected: "
          + EpidConstants.EPID2X_GID_SIZE);
      throw new IOException();
    }
    Epid20eB eb;
    try {
      eb = new Epid20eB(groupId);
      return eb.getArray();
  ...
  /**
   * Verifies Epid 2.0 signature using online verification. Only support verification of
   * epid 2.0 signatures
   *
  ...
  private int verify20SignatureOnline(byte[] epidGroupNo, int hashAlg, byte[] signature,
    ...
    EpidOnlineVerifier verifier = new EpidOnlineVerifier();
    return verifier.verifyOnline(EpidVersion.EPID_2_0, epidGroupNo, msg, signature, this);
  }
   ...
   * @return status
   */
  public int verify20Signature(byte[] gid, int hashAlg, byte[] signature, byte[] msg) {
  ...
  private int verify1xSignatureOnline(EpidVersion version, byte[] epidGroupNo, byte[] signature,
      byte[] message, byte[] nonce, byte[] taId) {
    ...
    EpidOnlineVerifier epidOnlineVerifier = new EpidOnlineVerifier();
    return epidOnlineVerifier.verifyOnline(version, epidGroupNo, adaptedMessage, signature, this);
  }
   ...
   * @return status
   */
  public int verify1xSignature(EpidVersion version, byte[] gid, byte[] signature, byte[] message,
  ...
  /**
   * Verifies Epid 1.0 signature. Only support verification of epid 1.0 signatures
   *
   ...
   * @return status
   */
  public int verify10Signature(byte[] epidGroupNo, byte[] signature, byte[] message, byte[] nonce,
      byte[] taId) {
    // epiddebug-mlog.info("TP: verify 10 signature");
    return verify1xSignature(EpidVersion.EPID_1_0, epidGroupNo, signature, message, nonce, taId);
  }

  /**
   * Verifies Epid 1.1 signature. Only support verification of epid 1.1 signatures
   *
   * @param epidGroupNo epidGroupNo is a 32 bit value
   * @param signature   - the signature to check
   * @param message     - the messageBytes that was signed
   ...
   * @return status
   */
  public int verify11Signature(byte[] epidGroupNo, byte[] signature, byte[] message, byte[] nonce,
      byte[] taId) {

    return verify1xSignature(EpidVersion.EPID_1_1, epidGroupNo, signature, message, nonce, taId);
  }
  ...
  private byte[] constructMessage_1_0(byte[] originalMessageBytes, int offset, int length,
    ...
    byte[] adaptedMessage =
        new byte[taIdLength.length + taId.length + nonce.length + originalMessageBytes.length];
    ...
    System.arraycopy(nonce, 0, adaptedMessage, taIdLength.length + taId.length, nonce.length);
    System.arraycopy(originalMessageBytes, offset, adaptedMessage,
