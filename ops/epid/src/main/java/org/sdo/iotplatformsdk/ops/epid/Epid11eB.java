...
/**
 * Handles the eB calculations for Epid version 1.0 and 1.1. The main rule is that the SigRl MUST be
 * either signed or ignored. SigRls with 0 entries are represented as zero length sigRls.
 */
public class Epid11eB {
   ...
   */
  public byte[] getArray() throws IOException, InterruptedException, TimeoutException {
    ...
    final EpidOnlineMaterial epidOnlineMaterial = new EpidOnlineMaterial();
    grpCertSigma11Online = epidOnlineMaterial.readEpidRestService(grpId,
        EpidLib.EpidVersion.EPID_1_1, EpidLib.MaterialId.PUBKEY_CRT);
    if (grpCertSigma11Online != null) {
      mlog.debug("grpCertSigma11Online read : " + grpCertSigma11Online.length);
    }

    grpCertSigma10Online = epidOnlineMaterial.readEpidRestService(grpId,
        EpidLib.EpidVersion.EPID_1_1, EpidLib.MaterialId.PUBKEY_CRT_BIN);
